...
function draw(echarts: any, id: string, name: string) {
  ...
  inst.setOption({
    ...
    title: { text: name, left: 6, top: 4, textStyle: { color: '#9fb3c8', fontSize: 12 } },
...
<template>
  <div style="display:grid;grid-template-columns:1fr;gap:8px">
    <div v-if="!echartsAvailable" style="border:1px solid #1b2a44;border-radius:6px;padding:8px;color:#9fb3c8;background:#0e1628">
      传感器面板占位（未安装 echarts）。将显示：IMSI 新现/趋势、雷达速度-距离、震动波形。
    </div>
    <div v-else style="display:grid;grid-template-columns:1fr;gap:8px">
      <div id="chart_radar" style="height:140px;border:1px solid #1b2a44;border-radius:6px;background:#0e1628"></div>
      <div id="chart_seismic" style="height:140px;border:1px solid #1b2a44;border-radius:6px;background:#0e1628"></div>
      <div id="chart_imsi" style="height:140px;border:1px solid #1b2a44;border-radius:6px;background:#0e1628"></div>
    </div>
  </div>
</template>
